"""Modern, user-friendly UI for DSG Downloader with terminal fallback support."""
...
def prompt_multichoice(
    ...
    user_input = input("Enter numbers separated by commas (or press Enter for defaults): ").strip()
    if not user_input and defaults:
        return set(defaults)
...
def get_user_selection() -> Dict[str, Any]:
    """Entry point that routes to GUI or Terminal CLI based on USE_UI in .env."""
    saved_data = {
    ...
    if not USE_UI:
        print("USE_UI is false. Bypassing GUI and running in Terminal CLI mode...")
        return _terminal_fallback(saved_data)

    try:
        return _launch_gui_selection(saved_data)
    except ImportError:
        print("CustomTkinter not installed or GUI unavailable. Falling back to terminal.")
        return _terminal_fallback(saved_data)
    except Exception:
        import traceback
...
def _launch_gui_selection(saved_data: Dict[str, Any]) -> Dict[str, Any]:
    """Launches CustomTkinter GUI interface."""
    app = ctk.CTk()
    ...
    ctk.CTkLabel(prep_frame, text="Divine Service Prep", font=("Arial", 14, "bold")).pack(pady=10)
    ...
    for o in prep_options:
        ctk.CTkCheckBox(prep_frame, text=o, variable=prep_vars[o]).pack(anchor="w", padx=10, pady=5)

    bible_frame = ctk.CTkFrame(prep_tab)
    bible_frame.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
    ctk.CTkLabel(bible_frame, text="Bible Readings", font=("Arial", 14, "bold")).pack(pady=10)
    ...
    for o in bible_options:
        ctk.CTkCheckBox(bible_frame, text=o, variable=bible_vars[o]).pack(anchor="w", padx=10, pady=5)
    ...
    ctk.CTkLabel(full_dsg_frame, text="Full DSG", font=("Arial", 14, "bold")).pack(pady=10)
    full_lang_vars = {
        l: ctk.IntVar(value=1 if l in saved_data["full_dsg_langs"] else 0)
        for l in lang_options
    }
    for l in lang_options:
        ctk.CTkCheckBox(full_dsg_frame, text=l, variable=full_lang_vars[l]).pack(anchor="w", padx=10, pady=5)

    se_dsg_frame = ctk.CTkFrame(editions_tab)
    se_dsg_frame.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
    ctk.CTkLabel(se_dsg_frame, text="Special Edition DSG", font=("Arial", 14, "bold")).pack(pady=10)
    se_lang_vars = {
        l: ctk.IntVar(value=1 if l in saved_data["se_dsg_langs"] else 0)
        for l in lang_options
    }
    for l in lang_options:
        ctk.CTkCheckBox(se_dsg_frame, text=l, variable=se_lang_vars[l]).pack(anchor="w", padx=10, pady=5)
    ...
    ctk.CTkLabel(districts_frame, text="Districts", font=("Arial", 14, "bold")).pack(pady=10)
    districts = [
        "British Columbia", "Alberta", "Saskatchewan", "Manitoba",
        "Northern Ontario", "Kitchener", "Hamilton", "Toronto", "Eastern Canada"
    ]
    district_vars = {
        d: ctk.IntVar(
            value=1 if d in (saved_data["schedules_sub"].get("District Serving Schedules", [])) else 0
        )
        for d in districts
    }
    for d in districts:
        ctk.CTkCheckBox(districts_frame, text=d, variable=district_vars[d]).pack(anchor="w", padx=10, pady=5)

    youth_seniors_frame = ctk.CTkFrame(schedules_tab)
    youth_seniors_frame.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
    ctk.CTkLabel(youth_seniors_frame, text="Youth and Seniors", font=("Arial", 14, "bold")).pack(pady=10)
    youth_var = ctk.IntVar(value=1 if "Youth Schedules" in saved_data["schedules_chosen"] else 0)
    seniors_var = ctk.IntVar(value=1 if "Seniors Schedules" in saved_data["schedules_chosen"] else 0)
    ctk.CTkCheckBox(youth_seniors_frame, text="Youth", variable=youth_var).pack(anchor="w", padx=10, pady=5)
    ctk.CTkCheckBox(youth_seniors_frame, text="Seniors", variable=seniors_var).pack(anchor="w", padx=10, pady=5)
    ...
    nacc_var = ctk.IntVar(value=1 if "NACC Calendars" in saved_data["schedules_chosen"] else 0)
    ctk.CTkCheckBox(nacc_frame, text="NACC Calendar", variable=nacc_var).pack(anchor="w", padx=10, pady=5)
    
    # --- Ministers Tab ---
    ministers_tab = tab_view.tab("Ministers")
    current_ministers = saved_data.get("minister_colors") or {}
    
    minister_frame_container = ctk.CTkFrame(ministers_tab)
    minister_frame_container.pack(padx=10, pady=10, fill="both", expand=True)
    ctk.CTkLabel(minister_frame_container, text="Minister Highlights", font=("Arial", 16, "bold")).pack(pady=10)
    entry_frame = ctk.CTkFrame(minister_frame_container)
    entry_frame.pack(pady=5, padx=10, fill="x")
    name_entry = ctk.CTkEntry(entry_frame, placeholder_text="Enter Minister Name...")
    name_entry.pack(side="left", fill="x", expand=True, padx=(0, 10))
    color_preview = ctk.CTkLabel(entry_frame, text="", width=40, height=28, corner_radius=6)
    ...
    def on_submit():
        ...
        if dist_selection:
            ...
            schedules_sub["District Serving Schedules"] = list(dist_selection)
        
        result.update(
            {
                "selections": selections,
                "bible_reading_langs": {o for o, v in bible_vars.items() if v.get()},
...
def _terminal_fallback(saved_data: Dict[str, Any]) -> Dict[str, Any]:
    """Fallback CLI prompt or configuration loader when GUI is disabled/unavailable."""
    print("\n--- DSG Downloader Terminal Mode ---")
    
    use_saved = input("Use saved choices from .env? [Y/n]: ").strip().lower()
    if use_saved not in ["n", "no"]:
        print("Loaded configurations from .env successfully.")
        return saved_data

    # Prompt user in terminal if they explicitly opt out of saved .env defaults
    prep_options = ["English", "French", "Audio", "References", "Transcript"]
    selections = prompt_multichoice("Select Divine Service Prep options:", prep_options, saved_data["selections"])

    schedules_options = ["District Serving Schedules", "Youth Schedules", "Seniors Schedules", "NACC Calendars"]
    schedules_chosen = prompt_multichoice("Select Schedules to download:", schedules_options, saved_data["schedules_chosen"])
    ...
    if "District Serving Schedules" in schedules_chosen:
        ...
        default_dists = saved_data["schedules_sub"].get("District Serving Schedules", ["Kitchener"])
        chosen_dists = prompt_multichoice("Select District Locations:", districts, default_dists)
        schedules_sub["District Serving Schedules"] = list(chosen_dists)

    result = {
        "selections": selections,
        "bible_reading_langs": saved_data["bible_reading_langs"],
        "full_dsg_langs": saved_data["full_dsg_langs"],
        "se_dsg_langs": saved_data["se_dsg_langs"],
        "schedules_chosen": schedules_chosen,
        "schedules_sub": schedules_sub,
        "minister_colors": saved_data["minister_colors"],
        "highlight_opacity": 0.5,
    }
    return result
